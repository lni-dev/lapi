...
public class GuildStoreChannel extends Channel implements GuildChannel {
    ...
    public GuildStoreChannel(@NotNull LApi lApi, @NotNull Snowflake id, @NotNull ChannelType type, @NotNull Data data) throws InvalidDataException {
        ...
        if (name == null) {
            throw new InvalidDataException("field '" + NAME_KEY + "' missing or null in GuildTextChannel with id:" + getId());
        } else if (guildId == null) {
            throw new InvalidDataException("field '" + GUILD_ID_KEY + "' missing or null in GuildTextChannel with id:" + getId());
        } else if (position == -1) {
            throw new InvalidDataException("field '" + POSITION_KEY + "' missing or -1 in GuildTextChannel with id:" + getId());
        }
